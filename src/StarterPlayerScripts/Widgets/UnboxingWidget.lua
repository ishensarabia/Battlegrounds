...
--Constants
local FRAME_SIZE = UDim2.fromScale(0.166, 1)
...
local function DisplayReward(chosenReward: table, rewardType)
	...
	if rewardType == "Skin" then
		warn(chosenReward)
		WidgetController:CreateSkinFrame(chosenReward, rewardFrame.ItemFrame)
	...
	if rewardType == "Emote" then
		local emoteFrame = WidgetController:CreateEmoteFrame(chosenReward,  rewardFrame.ItemFrame)
	...
	ButtonWidget.new(rewardFrame.CloseButtonFrame, function()
		--Tween the reward frame to its original size
...
function UnboxingWidget:Initialize(button: GuiButton, callback, customSoundName: string, params: table)
	...
	StoreService.OpenCrateSignal:Connect(
		function(crate: table, rewardChosen: table, cratesLeft: number, crateName: string, unboxingTime: number)
...
function UnboxingWidget:OpenCrate(crate: table, chosenReward: table, unboxTime: number)
	...
	for i = 1, numItems do
		...
		if crate.Type == "Skin" then
			WidgetController:CreateSkinFrame(randomItemChosen, unboxingFrame.ItemsFrame.ItemsContainer):andThen(function(skinFrame)
				skinFrame.Size = FRAME_SIZE
			end)
		end
		if crate.Type == "Emote" then
			local emoteFrame = WidgetController:CreateEmoteFrame(randomItemChosen, unboxingFrame.ItemsFrame.ItemsContainer)
			emoteFrame.Size = FRAME_SIZE
		end
	end
